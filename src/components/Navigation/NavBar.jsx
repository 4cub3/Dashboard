...
const NavBar = () => {
  ...
  return (
    <>
       <Button className='text-[2.5rem] text-gray-500  smd:hidden absolute top-[13.5rem] sm:top-44 smm:top-[9.5rem] right-[2.5rem] sm:right-20 z-[1000]' onclick={()=>{setNavExpand(prev => !prev)}}><FaBars/></Button>
    <nav className={`bg-primary dark:bg-slate-950 absolute h-screen overflow-y-scroll z-[900] smd:relative flex flex-col flex-3 overflow-hidden  p-12  ${navExpand ? '' : '-translate-x-full smd:translate-x-0'} transition-translate duration-300`}>
        {/* Nav button*/}
